...
interface PrintSweetSummaryProps {
  orders: Order[];
}

export const PrintSweetSummary = forwardRef<
  HTMLDivElement,
  ...
  const productSummary = orders.reduce(
    (acc, order) => {
      order.order_items.forEach((item) => {
        ...
        if (item.product.category_id === 5 || item.product.category_id === 17) {
  ...
  return (
    <div ref={ref} style={{ fontSize: "12px", margin: "0 10px" }}>
      <h2 className="text-xl font-bold mb-4">
        Výroba dortů, zákusků a čajového ({dateRange})
      </h2>
...
PrintSweetSummary.displayName = "Výroba zákusků a čajového";
