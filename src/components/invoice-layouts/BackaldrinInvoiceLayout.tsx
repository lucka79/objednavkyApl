...
interface BackaldrinInvoiceLayoutProps {
  items: any[];
  onUnmap?: (itemId: string) => void;
}

export function BackaldrinInvoiceLayout({
  items,
  onUnmap,
}: BackaldrinInvoiceLayoutProps) {
  return (
        ...
        <tbody className="bg-white">
          {items?.map((item: any, idx: number) => {
            ...
            return (
              <tr
                key={idx}
                className={`border-b border-gray-200 hover:bg-amber-50/30 ${
                  isLowConfidence
              ...
              >
                <td className="px-3 py-2 border-r border-gray-200">
                  <code className="text-xs bg-amber-100 text-gray-700 px-2 py-0.5 rounded font-mono">
                    {productCode || "???"}
                  </code>
                </td>
                ...
                <td className="px-3 py-2 text-right text-xs text-gray-600 border-r border-gray-200">
                  {vatRate ? `${vatRate}%` : "-"}
                </td>
